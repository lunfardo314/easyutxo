...
func genesisOutput(genesisPublicKey ed25519.PublicKey, initialSupply uint64, ts uint32) (*Output, OutputID) {
	...
	out := NewOutput()
	out.Amount = initialSupply
	out.Timestamp = ts
	out.Address = AddressFromED25519PubKey(genesisPublicKey)
...
func (u *State) updateLedger(ctx *ValidationContext) {
	...
	ctx.ForEachOutput(Path(TransactionBranch), func(o *Output, outputPath lazyslice.TreePath) bool {
